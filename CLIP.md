...
`code/clip.py`에서 Figure 3이랑 같은 대칭 InfoNCE를 돌려 봤다.

이미지/텍스트 특징을 L2 normalize한 뒤 내적이 cosine이다. $\tau$가 커지면 softmax가 날카로워진다. loss를 이미지→텍스트, 텍스트→이미지 양쪽으로 평균내서 한쪽만 맞추는 걸 막는다.

배치가 3이면 logits는 3x3이고 정답은 대각선 (0,1,2)다.

실행:

```
python papers/code/clip.py
```
